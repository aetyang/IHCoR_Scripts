...
# 14 Apr 2025: modifications to include no of succesful daytime and nighttime total_readings and
#changes to match output to that from the spacelabs PDFs
...
file_name=f"omron_24_7_results2025_kenya.csv"
...
for pdf_file in pdf_files:
    if pdf_file.endswith(".pdf"):
        ...
        try:
            ...
            successful_readings_pattern = r'readings \s+(\d+)\s+(\-)\s+(\d+)\s+(\d+)'
            successful_readings_match= re.search(successful_readings_pattern, first_page_text)
            print(successful_readings_match)
            awake_readings=int (successful_readings_match.group(3))
            print(day_readings)
            asleep_readings=int(successful_readings_match.group(4))
            ...
            table_p2_info = read_pdf(pdf_path, pages=[2], format="csv",
            lattice = True,area=[(75, 372, 1200, 500)], )
            print (table_p2_info)

            array_info = np.array(table_p2_info)
            print (array_info)
            ...
            reshaped_info = array_info.reshape(-1, array_info.shape[-1])
            ...
            table_p2_info_df = pd.DataFrame(reshaped_info)
            print (table_p2_info_df)

            table_p2 = pd.concat([table_p2_times_df, table_p2_bps_df,table_p2_info_df], axis=1)
            print (table_p2)
            # Remove all colons in the entire DataFrame
            table_p2 = table_p2.replace(':', '', regex=True)
            print (table_p2)
            # filter to remove measurements that were taken before start Time exclude last column ("information")
            table_p2.iloc[:, :-1] = table_p2.iloc[:, :-1].apply(pd.to_numeric, errors='coerce')

            #table_p2 = table_p2.apply(pd.to_numeric, errors='coerce')
            print (table_p2)

            column_names_p2= ['Time','sbp','dbp','HR', 'Info']
            ...
            table_p3_info = read_pdf(pdf_path, pages=[3], format="csv",
            lattice = True,area=[(95, 372, 1200, 500)], )
            print (table_p3_info)

            array_info_p3=np.array(table_p3_info)
            ...
            reshaped_info_p3 = array_info_p3.reshape(-1, array_info_p3.shape[-1])
            ...
            table_p3_info_df = pd.DataFrame(reshaped_info_p3)
            print (table_p3_info_df)

            table_p3 = pd.concat([table_p3_times_df, table_p3_bps_df, table_p3_info_df], axis=1)
            print (table_p3)
            ...
            print (table_p3)
            # filter to remove measurements that were taken before start Time exclude last column ("information")
            table_p3.iloc[:, :-1] = table_p3.iloc[:, :-1].apply(pd.to_numeric, errors='coerce')
            print (table_p3)

            column_names_p3= ['Time','sbp','dbp','HR', 'Info']
            ...
            total_attempts = combined_tables.shape[0]

            print (total_count)

            print (total_attempts)

            failure_count = combined_tables["Info"].str.contains("failed reading", na=False).sum()
            ...
            column_names = ['Day','Time','Sys','Dia','HR', 'Info']
            ...
            serial_number = 247  # Omron 24/7 indentifier
            #print (study_id)
            ...
            if combined_tables.shape[1] >= 5:
                ...
                metadata = pd.DataFrame({
                    ...
                    'ttl_attempted_readings': [total_attempts] * selected_columns.shape[0],
